...
1. Update reflow scheduling to consider maintenance work orders as occupied intervals when searching for the next available slot.
2. Update constraint checker to flag overlaps with maintenance work orders and keep maintenance orders fixed.
3. Add or update a scenario that forces a conflict with a maintenance work order and assert expected reflow/validation outcomes in tests.
4. Document the new behavior and any edge cases (multiple maintenance sources, shift boundaries).
